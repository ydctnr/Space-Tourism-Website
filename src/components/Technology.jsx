...
const Technology = () => {
  ...
  return (
    ...
    >
        ...
        <div className='flex flex-col-reverse max-lg:gap-10 lg:flex-row lg:justify-between max-lg:text-center justify-center items-center max-lg:items-center min-h-screen md:pt-36 pt-20 lg:pl-[12.5%]'>

          <div className='flex flex-col lg:flex-row'>

            <div className='m-auto left-0 right-0 lg:pr-10'>
              <ul className='flex flex-col max-lg:flex-row gap-6 lg:gap-8 cursor-pointer font-bellefair'>
                <li aria-label="Launch Vehicle"
                    className={`w-14 h-14 rounded-full border-[0.063em] border-gray-500 flex justify-center items-center text-xl ${selectedTab === 1 ? 'bg-white text-black' : 'text-white'}`}
                    onClick={() => setSelectedTab(1)}>1
                </li>
                <li aria-label="Spaceport"
                    className={`w-14 h-14 rounded-full border-[0.063em] border-gray-500 flex justify-center items-center text-xl ${selectedTab === 2 ? 'bg-white text-black' : 'text-white'}`}
                    onClick={() => setSelectedTab(2)}
                >2</li>
                <li aria-label="Space Capsule"
                    className={`w-14 h-14 rounded-full border-[0.063em] border-gray-500 flex justify-center items-center text-xl ${selectedTab === 3 ? 'bg-white text-black' : 'text-white'}`}
                    onClick={() => setSelectedTab(3)}
                >3</li>
              </ul>
            </div>

            <div className='grid grid-flow-row gap-2 max-lg:mt-4 tracking-widest lg:text-left'>
              <h6 className='text-lg font-barlow text-gray-500 uppercase'>The terminology...</h6>
              <h2 className='text-4xl font-bellefair text-white uppercase mt-2'>{content[selectedTab - 1].title}</h2>
              <p className='text-gray-400 text-sm font-medium tracking-wide leading-6 mt-2 max-w-sm max-lg:max-w-xs'>{content[selectedTab - 1].description}</p>
            </div>

          </div>

            <img
              className='max-lg:max-w-md md:max-w-4xl lg:w-[30%] xl:w-[40%]'
